jQuery(function() {
  ...
  jQuery(
    ".create-group__input, .create-group__textarea, .create-group__select"
  ).on("change keyup paste", function(e) {
    var $this = jQuery(this);
    if ($this.val() != "" || $this.val() == "0") {
      $this.removeClass("create-group__input--error");
      $this
        .next(".form__error-container")
  ...
  jQuery("#create-group-form").one("submit", function(e) {
    ...
    jQuery(":input[required]").each(function(index, element) {
      ...
      if (
        $ele.val() == "" ||
        $ele.val() == "0" ||
        ($ele.is(":checkbox") && $ele.prop("checked") === false)
      ) {
        error = true;
  ...
  jQuery(".group__join-cta").click(function(e) {
    ...
    var url =
      $this.text() == "Join Group"
        ? "/wp-admin/admin-ajax.php?action=join_group"
        : "/wp-admin/admin-ajax.php?action=leave_group";
