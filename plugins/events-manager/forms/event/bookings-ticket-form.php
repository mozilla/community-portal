			...
			<div class="ticket-dates-from">
				...
				<div class="ticket-dates-from-normal">
					...
					<input type="hidden" name="em_tickets[<?php echo esc_attr( $col_count ); ?>][ticket_start]" class="em-date-input ticket_start" value="
						...
						if ( ! empty( $em_ticket->ticket_start ) ) {
							echo esc_attr( $em_ticket->start()->format( 'Y-m-d' ) );
						} else {
							echo esc_attr( '' );  }
						?>
					" />
				...
				<div class="ticket-dates-from-recurring">
					...
					if ( ! empty( $em_ticket->ticket_meta['recurrences']['start_days'] ) && is_numeric( $em_ticket->ticket_meta['recurrences']['start_days'] ) ) {
						echo esc_attr( absint( $em_ticket->ticket_meta['recurrences']['start_days'] ) );}
					?>
					" />
				...
				<?php
				if ( ! empty( $em_ticket->ticket_start ) ) {
					echo esc_attr( $em_ticket->start()->format( em_get_hour_format() ) );
				} else {
					echo esc_attr( '' );
				}
				?>
			...
			<div class="ticket-dates-to">
				...
				<div class="ticket-dates-to-normal">
					...
					if ( ! empty( $em_ticket->ticket_end ) ) {
						echo esc_attr( $em_ticket->end()->format( 'Y-m-d' ) );
					} else {
						echo esc_attr( '' );
					}
					?>
				...
				<div class="ticket-dates-to-recurring">
					...
					<?php
					if ( isset( $em_ticket->ticket_meta['recurrences']['end_days'] ) && false !== $em_ticket->ticket_meta['recurrences']['end_days'] ) {
						echo esc_attr( absint( $em_ticket->ticket_meta['recurrences']['end_days'] ) );}
					?>
					" />
					<?php esc_html_e( 'day(s)', 'community-portal' ); ?>
				...
				<?php
				if ( ! empty( $em_ticket->ticket_end ) ) {
					echo esc_attr( $em_ticket->end()->format( em_get_hour_format() ) );
				} else {
					esc_attr( '' );
				};
				?>
		...
		if ( ! $em_ticket->ticket_members ) :
			...
			<div>
				...
				foreach ( $event_roles->roles as $event_role => $role_data ) {
					...
					<?php
					if ( in_array( $role, $em_ticket->ticket_members_roles, true ) ) {
						echo esc_attr( 'checked="checked"' );}
					?>
