...
<div class="em-search-wrapper row">
	<div class="em-events-search em-search col-lg-7 col-sm-12">
		...
		<form method="GET" action="<?php echo !empty($args['search_url']) ? esc_url($args['search_url']) : EM_URI; ?>" class="events__form">
			...
			<input type="text" name="em_search" class="events__search-input" placeholder="<?php _e('Search events', 'community-portal'); ?>" value="<?php echo esc_attr($args['search']); ?>" />
			</div>
			<input type="submit" class="events__search-cta" value="<?php _e('Search', 'community-portal'); ?>" />
		</form>
	</div>
