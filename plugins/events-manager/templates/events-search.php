...
<div class="em-search-wrapper row">
	<div class="em-events-search em-search col-lg-7 col-sm-12">
		<?php
			...
			$query_country      = isset( $_GET['country'] ) && strlen( sanitize_title_for_query( wp_unslash( $_GET['country'] ) ) ) > 0 ? sanitize_text_field( wp_unslash( $_GET['country'] ) ) : false;
		...
		?>
			...
			if ( $query_country ) {
				?>
				<input type="hidden" name="country" value="<?php echo esc_attr( ucwords( urldecode( $query_country ) ) ); ?>" />
				<?php
			}
			if ( $query_language ) {
				?>
